...
void MeanShift::ErrorHandler(std::string className, std::string methodName, std::string errmsg)
{
	
	//store trace into error message
	strcpy(ErrorMessage, className.c_str());
	strcat(ErrorMessage, "::");
	strcat(ErrorMessage, methodName.c_str());
	strcat(ErrorMessage, " Error: ");
	
	//store message into error message
	strcat(ErrorMessage, errmsg.c_str());
