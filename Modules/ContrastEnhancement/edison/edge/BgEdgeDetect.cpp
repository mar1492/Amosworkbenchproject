...
BgEdgeDetect::BgEdgeDetect(int filtDim)
{
    ...
    permConf_ = nullptr;
    permRank_ = nullptr;
    permNmxRank_ = nullptr;
    permNmxConf_ = nullptr;  
...
{
//     std::cout << "Deleting BgEdgeDetect" << std::endl;
    if (havePerm_==true)
    {
        delete [] permGx_;
        delete [] permGy_;
...
float BgEdgeDetect::SquareComp(float x0, float y0, float x, float y)
{
    ...
    static thread_local float tret;
...
void BgEdgeDetect::GenerateMaskAngle(double* a,double theta) {
    static thread_local int sflag;
    static thread_local int i,j,k;
    static thread_local double cval[4];
    static thread_local double corner[2][4];
    static thread_local double sinv,cosv;
    static thread_local double intrs[2][4];
    static thread_local int scor[4],nscor[4];
    static thread_local int sind,rowind,colind;
    static thread_local double cordi[2][4];
    static thread_local int lsigind,corin;
    static thread_local int sigind[4];
    static thread_local double diffin[2];
    static thread_local double comcoor;
                                ...
                                int nMin, int nmxType, int hystTypeHigh, int hystTypeLow)
{
    ...
    if(permConf_ == nullptr)
        permConf_ = new float[x_*y_];
    if(permRank_ == nullptr)
        permRank_ = new float[x_*y_];
    if(permNmxRank_ == nullptr)
        permNmxRank_ = new float[x_*y_];
    if(permNmxConf_ == nullptr)
...
void BgEdgeDetect::CompRanks(float* strength, float* ranks)
{
    ...
    irra--;
    // JM Cuadra
    // A full zero image ranks -> nan because irra = 0
    if(irra > 0) {
        for (ii=0; ii<x_*y_; ii++) {
            ranks[ii] /= irra;
        }
...
void BgEdgeDetect::NewNonMaxSupress(float* rank, float* conf, float* grx, float* gry, float* nmxRank, float* nmxConf, float (BgEdgeDetect::*feval)(float, float, float, float))
{
    int i,j;
    float* itr;
    float* itc;
