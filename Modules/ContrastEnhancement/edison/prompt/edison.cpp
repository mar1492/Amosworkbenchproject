...
EDISON::~EDISON( void )
{
//     std::cout << "Deleting Edison" << std::endl;
...
void EDISON::writeEdges(unsigned char *edgesMat, int *dataPoints, int n)
{
    if(dataPoints) {
        int i, j/*, x, y*/;
        for(i = 0; i < n; i++) {
            for(j = 0; j < dim_; j++) {
                edgesMat[dim_*dataPoints[i]+j] = 255;
            }
        }
    }    
}
...
void EDISON::SetParametersNew(QMap< QString, QString > parameters)
{
...
int EDISON::EdgeDetectNew(float* gradMapMat, uchar* edgesMapMat, float* confMapMat)
{   
    //make sure an input image was defined
    if(!inputDefined_) return EXE_INPUT_UNDEFINED;
...
//     CmCDisplayProgress = false;
    ...
    //obtain parameters
    int gradWindRad     = parameters["GradientWindowRadius"].toInt();
    int minLength       = parameters["MinimumLength"].toInt();
    float nmxRank       = parameters["NmxRank"].toDouble();
    float nmxConf       = parameters["NmxConf"].toDouble();
    int   nmxType       = parameters["NmxType"].toInt();
    float hystHighRank  = parameters["HysterisisHighRank"].toDouble();
    float hystHighConf  = parameters["HysterisisHighConf"].toDouble();
    int   hystHighType  = parameters["HysterisisHighType"].toInt();
    float hystLowRank   = parameters["HysterisisLowRank"].toDouble();
    float hystLowConf   = parameters["HysterisisLowConf"].toDouble();
    int   hystLowType   = parameters["HysterisisLowType"].toInt();    
    ...
    double *ptsX, *ptsY;
    BgEdgeDetect edgeDetector(gradWindRad);
//     BgEdgeDetect* edgeDetector = new BgEdgeDetect(gradWindRad);
    if(hystHighType == CURVE_CUSTOM) {
        n = 2; // TODO adapt to QList
        ptsX = new double [n];
        ptsY = new double [n];
        ptsX[0] = parameters["CustCurveHysterisisHigh_00"].toDouble();
        ptsX[1] = parameters["CustCurveHysterisisHigh_10"].toDouble();
        ptsY[0] = parameters["CustCurveHysterisisHigh_01"].toDouble();
        ptsY[1] = parameters["CustCurveHysterisisHigh_11"].toDouble();
        edgeDetector.SetCustomHigh(ptsX, ptsY, n);
        delete [] ptsX;
        delete [] ptsY;
    }
    if(hystLowType == CURVE_CUSTOM) {
        n = 2; // TODO adapt to QList n could be > 2, see help/specification.html#vertexlist
        ptsX = new double [n];
        ptsY = new double [n];
        ptsX[0] = parameters["CustCurveHysterisisLow_00"].toDouble();
        ptsX[1] = parameters["CustCurveHysterisisLow_10"].toDouble();
        ptsY[0] = parameters["CustCurveHysterisisLow_01"].toDouble();
        ptsY[1] = parameters["CustCurveHysterisisLow_11"].toDouble();
        edgeDetector.SetCustomLow(ptsX, ptsY, n);
        delete [] ptsX;
        delete [] ptsY;
    }
    ...
    //edge detect the input image
    BgEdgeList   edgeList;
    edgeDetector.DoEdgeDetect(&inputImage, &edgeList, nmxRank, nmxConf, hystHighRank, hystHighConf, hystLowRank, hystLowConf, minLength, nmxType, hystHighType, hystLowType);  
    if(edgesDefined_) delete [] edges_;
    edges_ = new int [height_ * width_];
    int *edgex, *edgey;
    edgex = new int [height_ * width_];
...
/*    std::cout << "height_" << height_ << std::endl;
    ...
    memcpy(gradMapMat, gradMap_, height_ * width_ * sizeof(float));
    if(confMapMat != nullptr) 
        memcpy(confMapMat, confMap_, height_ * width_ * sizeof(float));
    
    if(edgesMapMat != nullptr) {
        writeEdges(edgesMapMat, edges_, numEdges_);
    }
    //delete edgeDetector;
...
void EDISON::setInputImage(unsigned char * inputImage, int height, int width, int dim)
{
