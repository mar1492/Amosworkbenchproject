...
public:
    ...
    void SetParametersNew(QMap< QString, QString > parameters);
    ...
    // *** Edge Detection ***
    int EdgeDetect( void );
    int EdgeDetectNew(float* gradMapMat, uchar* edgesMapMat, float* confMapMat);
    void writeEdges(unsigned char *edgesMat, int *dataPoints, int n);
    
    void setInputImage(unsigned char * inputImage, int height, int width, int dim);
...
private:
    ...
    float *gradMap_;
    float *confMap_;    float *weightMap_;
    float *custMap_;
